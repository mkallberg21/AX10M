 ...
 * marked TODO(ax10m). The shape, capability matrix, and safety invariants are real.
 *
...
export class StripeAdapter implements ProcessorAdapter {
  ...
  constructor(private readonly config: StripeAdapterConfig) {
    // TODO(ax10m): construct the Stripe SDK client with restricted key +
  ...
  async ingestWebhook(raw: RawWebhook): Promise<CanonicalEvent[]> {
    // TODO(ax10m): verify signature with stripe.webhooks.constructEvent(
    //   raw.body, raw.headers['stripe-signature'], this.config.webhookSecret).
    // Reject on failure — never trust an unverified body.
    //
    // TODO(ax10m): switch on event.type and normalize into canonical events:
  ...
  async listOpenFailures(cursor: Cursor): Promise<OpenFailuresPage> {
    // TODO(ax10m): page stripe.invoices.list({ status: 'open', collection_method:
  ...
  async attemptCharge(
    ...
    // TODO(ax10m): create/confirm a PaymentIntent for invoice.amount against
    ...
    void method;
    throw new Error(
      `TODO(ax10m): StripeAdapter.attemptCharge not implemented (idemKey=${idempotencyKey})`,
    );
  }

  async fetchUpdatedCard(method: PaymentMethod): Promise<PaymentMethod | null> {
    // TODO(ax10m): read the PaymentMethod / network token to see if the card was
  ...
  async listPaymentMethods(customer: Customer): Promise<PaymentMethod[]> {
    // TODO(ax10m): stripe.paymentMethods.list({ customer: customer.processorRef,
  ...
  async pauseNativeDunning(subscription: Subscription): Promise<void> {
    // TODO(ax10m): disable Smart Retries / native dunning for this subscription
    //   so AX10M can take control. NOTE: in Phase 0 shadow mode we deliberately
    ...
    void subscription;
    throw new Error('TODO(ax10m): StripeAdapter.pauseNativeDunning not implemented');
  }
}
