...
export class BraintreeAdapter implements ProcessorAdapter {
  ...
  async ingestWebhook(raw: RawWebhook): Promise<CanonicalEvent[]> {
    ...
    // Dispute lifecycle. dispute_opened → the chargeback settled and pulled funds
    // (payment.reversed); dispute_won → we later won that chargeback and the funds came
    // back (payment.reversal_reverted, undoing the prior reversal). Both carry the same
    // <subject><dispute> shape and map back to the recovery the same way, so they share
    // one parser. dispute_lost is a no-op: the funds already left on dispute_opened, so
    // there's nothing further to reverse.
    if (kind === 'dispute_opened' || kind === 'dispute_won') {
      const parsed = this.parseDisputeReversal(subject, occurredAt, kind);
      if (!parsed) return [];
      const type = kind === 'dispute_won' ? 'payment.reversal_reverted' : 'payment.reversed';
      return [{ ...parsed.base, type, payload: parsed.reversal }];
    }
  ...
  /**
   * Parse a Braintree dispute webhook's <subject><dispute> into a canonical ReversalPayload
   * plus the shared event base, or null when the dispute isn't a mappable funds movement.
   *
   * Shared by dispute_opened (→ payment.reversed) and dispute_won (→ payment.reversal_reverted):
   * the two webhooks carry an identical <dispute>…<transaction>…</transaction></dispute> shape.
   *
   * The dispute's own <kind> is chargeback | retrieval | pre_arbitration. Only a true
   * chargeback moves funds (a settled reversal); retrieval/pre_arbitration are info requests /
   * escalations, so we don't emit for those (and a won on a non-chargeback has nothing to undo).
   *
   * CRITICAL back-reference: the disputed transaction's <order-id> is the invoice ref AX10M
   * stamped at charge time — attemptCharge sets `<order-id> = invoice.processorRef`, and
   * buildInvoice keys `id = ax10m_inv_${processorRef}`, so `ax10m_inv_${orderId}` reconstructs
   * the SAME invoice.id the recovery used. Braintree's dispute webhook exposes a trimmed
   * <transaction> (id, amount, created-at, order-id, purchase-order-number,
   * payment-instrument-subtype) with NO <subscription-id>, so <order-id> is the only reliable
   * link back to the recovery. If it's absent (Braintree renders it as a nil self-closing
   * element → xmlLeaf undefined), the disputed charge isn't one we can map to a recovery, so we
   * return null rather than fabricate an invoice id.
   */
  private parseDisputeReversal(
    subject: string,
    occurredAt: string,
    kind: string,
  ): { reversal: ReversalPayload; base: { id: string; merchantId: string; processorEventId: string; occurredAt: string } } | null {
    ...
    if ((xmlLeaf(dispute, 'kind') ?? '').toLowerCase() !== 'chargeback') return null;
