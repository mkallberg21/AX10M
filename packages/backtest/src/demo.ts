...
export function buildDemoData(opts: { seed?: number; nCustomers?: number } = {}): DemoData {
  ...
  return {
    ...
    meta: {
      ...
      seed,
      backtestVerdict:
        'The AX10M engine BEATS Stripe Smart Retries by ~13.8 pp on recovery rate in the backtest — via dead-credential ' +
        'recovery (Account Updater + backup-rail + dunning), a capability a blanket retry cannot copy. The win survives ' +
        'the fairness sweep. This is a MODELED (synthetic) result; a live holdout is what proves the magnitude.',
      note: 'Synthetic demo data generated by the world model. Not a real merchant; the win is MODELED, not live-verified — a live holdout is what turns it into a real bill. The retrained-model panel runs the REAL champion/challenger gate on a synthetic corpus.',
    },
  };
}
